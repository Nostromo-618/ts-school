...
promise and the TypeScript 6.0.3 checker pin, and a solid not-found experience,
all wired into routes, nav, and search.
...
as a teaching point, that this site's in-browser checker uses TypeScript 6.0.3
because TypeScript 7 ships no programmatic browser API.
...
- **THEN** they see ordered milestones covering ECMAScript evolution, Node.js,
  ...
  an honest note that the live checker remains on TypeScript 6.0.3
...
(paired JS-vs-TS lessons checked by a real compiler), who it is for (working
Node.js JavaScript developers), and why the in-browser checker is pinned to
TypeScript 6.0.3 rather than TypeScript 7.
...
#### Scenario: about names the checker constraint

- **GIVEN** a reader on `/about`
- **WHEN** they read the page
- **THEN** they learn that TypeScript 7 has no programmatic browser API and
  that this site therefore type-checks with TypeScript 6.0.3

### Requirement: not-found stays usable

Unknown paths MUST resolve to the existing not-found page. That page MUST offer
at least a link home and SHOULD also link into the curriculum map so a lost
...
`/history` and `/about` MUST be registered as standalone pages in the derived
nav tree's hand-written pages list and as named routes in `buildRoutes()`, so
the navbar lists them and the global search index includes them. Footer site
links MUST also reach them.

#### Scenario: navbar lists history and about

- **GIVEN** the derived nav tree
- **WHEN** its standalone pages are inspected
- **THEN** both `/history` and `/about` are present with titles and keywords

#### Scenario: search indexes the new pages

- **GIVEN** the search store built from the nav tree
- **WHEN** a reader queries for "history" or "about"
