...
`Track`, and `Lesson`. Every code field SHALL be a plain string; the model MUST
NOT contain a field whose contents are rendered as HTML. The module MUST import
`ExpectedDiagnostic` from `@/typecheck/types` and MUST NOT redefine it.
...
- **WHEN** an author declares a lesson with `id`, `title`, `tier`, `track`,
  `order`, `summary`, `prerequisites`, `keywords`, `problem`, `js`, `ts`, and
  `insight`
