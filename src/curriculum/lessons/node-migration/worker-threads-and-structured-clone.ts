...
export const lesson: Lesson = {
  ...
  problem:
    'Structured clone cannot carry functions; JS will not warn. Migration does not change Node\'s runtime — it surfaces the unions and module edges you already had to handle. Fix the seam "Worker threads" names before you rename the next hundred files.',
  solution:
    "Protocol types make illegal messages a compile error. Define request/response unions shared by both threads — same as a network protocol. Structured clone supports many built-ins but not functions or DOM nodes in Node workers the same way. Validate `unknown` messages at the edge; do not trust postMessage peers blindly. Public APIs first; loosen only where you can name the tradeoff.",
  js: {
